...
- **Chapters, not individual requirements.** The coverage matrix maps at chapter granularity (C1 through C12). Within each marked chapter, the applicable `Verify that…` requirements and their assurance Levels (L1 baseline to L3 advanced) depend on your system; consult the chapter text in AISVS 1.0. The exception is the agentic requirement-level pointers below, named where the mapping is precise enough to act on.
...
- **Agentic requirement-level pointers.** For LLM03 Excessive Agency, the C9 requirements that carry the defense are C9.2.3 (reversibility classification), C9.2.4 (enforce by class), and C9.2.10 (worst-case reversibility across chained calls). This is a starting point for the agentic entries, not a closed list; other agentic risks can be named the same way as the mapping matures.
