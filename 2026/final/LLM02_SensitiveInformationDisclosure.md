...
**6. Training-pipeline disclosure.** Gradient inversion with malicious aggregation (Boenisch et al., 2023), distillation, and synthetic-data carryover move examples into derived models. A 2026 attack class, **Differential Privacy Reversal via LLM Feedback**, uses an attacker-controlled LLM to provide structured feedback that progressively narrows a DP target's noise distribution and reconstructs individual training points across iterated queries — so fixed-epsilon DP is necessary but not sufficient without rate-limiting, query-pattern detection, and per-user budgets.
