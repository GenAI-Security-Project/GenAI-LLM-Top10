...
* **Amplified in agentic settings, covered by ASI.** Where the model acts as an *autonomous actor* — maintaining persistent memory across sessions, selecting tools, taking multi-step actions, coordinating with other agents, or operating with delegated authority — the amplified disclosure risk is addressed by the OWASP Top 10 for Agentic Applications. Examples in scope of ASI: persistent-memory injection across sessions, multi-agent worm propagation of exfiltration instructions, zero-click exfiltration by autonomous agents over multi-step tool chains, computer-use adjacent-content capture, and Model Context Protocol (MCP) tool poisoning. This entry treats those as the *agentic amplification* of foundational risks documented here, and points to ASI rather than reproducing their detail.
...
7. **RAG over-retrieval.** RAG systems that enforce authorization at the application layer *after* retrieval, rather than at the retrieval layer before content enters the model context, surface content across authorization boundaries. Per-document access-control list (ACL) propagation is the foundational control. Vector-authorization mechanics are addressed in **LLM08:2025 Vector and Embedding Weaknesses**; this entry addresses the disclosure outcome and its regulatory consequences.
